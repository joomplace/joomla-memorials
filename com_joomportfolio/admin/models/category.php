 ...
 * @since       1.6
 */
class JoomPortfolioModelCategory extends JModelAdmin
{
     ...
     * @since   1.6
     */
    public function getTable($type = 'Category', $prefix = 'JoomPortfolioTable', $config = array())
    {
    ...
    public function deleteItems($data)
    {
        ...
        if (!empty($data)) {
            for ($i = 0; $i < count($data); $i++) {
                ...
                $query->where('i.cat_id=' . (int)$data[$i]);
                $db->setQuery($query);
                $i_id = $db->loadObjectList();
                ...
                if ((int)$i_id[$i]->id) {
                    JoomPortfolioHelper::deleteDirectory(JPATH_SITE . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . 'joomportfolio' . DIRECTORY_SEPARATOR . (int)$i_id[$i]->id);
                }
            }
