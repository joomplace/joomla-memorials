...
<link rel="stylesheet" href="components/com_joomportfolio/assets/css/jquery.fileupload-ui.css">
  ...
  <div class="span7" style="float: right;">
    ...
    <tr class="template-download-3 fade in" style="display: table-row;">

            <td class="preview">
                <!-- <a target="_blank" href="<?php echo(JUri::root()) . 'images' . DIRECTORY_SEPARATOR . 'joomportfolio' . DIRECTORY_SEPARATOR . $item_id. DIRECTORY_SEPARATOR . 'audio' . DIRECTORY_SEPARATOR . $image->full ?>">Play Sound</a>-->
             <audio controls="" ><source src="<?php echo(JUri::root()) . 'images' . DIRECTORY_SEPARATOR . 'joomportfolio' . DIRECTORY_SEPARATOR . $item_id. DIRECTORY_SEPARATOR . 'audio' . DIRECTORY_SEPARATOR . $image->full ?>" type="audio/mpeg"></audio>
                <input type="hidden" name="jform[audio][img_path][]" value="<?php echo $image->full ?>">
            </td>
            <td>
                <table>
                    <tr>
                        <td class="title" style="width: 350px;"><input type="text" name="jform[audio][img_title][]" value="<?php echo $image->title ?>" style="width: 90%; min-height: 27px; text-align: center;" placeholder="<?php echo JText::_('COM_JOOMPORTFOLIO_IMAGE_TITLE'); ?>"/></td>
                        <td class="ordering" style="width: 10px;"><input type="text" name="jform[audio][img_ordering][]"  value="<?php echo $image->ordering ?>" style="width: 90%; min-height: 27px; text-align: center;" placeholder="0"/></td>

                        <td class="delete-audio" style="/*position: absolute; right: 90px;*/ background-color:#F9F9F9; border:none; min-width:144px;">
                            <span class="btn btn-danger ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" data-type="POST" data-url="index.php?option=com_joomportfolio&amp;task=uploader.deleteAudio&amp;id=<?php echo $image->item_id; ?>&amp;image=<?php echo $image->full ?>" role="button" aria-disabled="false">
...
<script id="template-upload-3" type="text/x-tmpl">
...
<script id="template-download-3" type="text/x-tmpl">

    {% console.log(o); for (var i=0, file; file=o.files[i]; i++) { %}
    {% if(file.message==undefined){  %}
    <tr class="template-download-3 fade in">
        {% if (file.error) { %}
        <td></td>
        <td class="name"><span>{%=file.name%}</span></td>
        ...
        {% } else { %}
        <td class="preview">
            <audio controls=""  name="media"><source src="{%=file.url%}" type="audio/mpeg"></audio>
            {% if (file.thumbnail_url) { %}
            <input type="hidden" name="jform[audio][img_path][]" value="{%=file.name%}" />
            {% } %}</td>
        <td>
            <table>
                <tr>
                    <td class="title" style="width: 350px;"><input type="text" name="jform[audio][img_title][]" value="{%=file.title%}" style="width: 90%; min-height: 27px; text-align: center;" placeholder="<?php echo JText::_('COM_JOOMPORTFOLIO_IMAGE_TITLE'); ?>" /></td>

                    <td class="ordering" style="width: 10px;"><input type="text" name="jform[audio][img_ordering][]" style="width: 90%; min-height: 27px; text-align: center;" placeholder="0" /></td>
                    <!--        <td class="size"><span>{%=o.formatFileSize(file.size)%}</span></td> -->

                    {% } %}
                    <td class="delete-audio" onclick="rowAudioDelete(this)" style=" background-color:#F9F9F9; border:none; min-width:144px;">
                        <span class="btn btn-danger ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" role="button" aria-disabled="false"
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        jQuery('#audio-form').fileupload({
            url: 'index.php?option=com_joomportfolio&task=uploader.addAudio&item_id=<?php echo $input->get('id') ?>',
            formData: {task: 'uploader.addAudio'},
            downloadTemplateId: 'template-download-3',
            previewSourceFileTypes: '/^image\/(wav|mp3)$/',
            uploadTemplateId: 'template-upload-3'
        });

        jQuery(".delete-audio").on('click', '.btn-danger', function (e) {
